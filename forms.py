...
class NewUserForm(FlaskForm):
    ...
    username = StringField('Username', validators=[InputRequired(message='Please enter a Username'), Length(
        min=1, max=20, message='Please select a valid length of between 1 and 20 characters')])
    ...
    email = StringField('Email Address', validators=[InputRequired(message='Please enter an email address'), Length(
        min=6, max=50, message='Please enter a valid length between 6 and 50 characters'), Email(message='Please enter a valid email address')])
